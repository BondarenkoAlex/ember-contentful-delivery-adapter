    ...
    describe('#findRecord', function () {
      ...
      it('model "asset"', function () {
        ...
        adapter.ajax = function (url, type, options) {
          console.log(url);
          expect(url).to.equal('https://cdn.contentful.com/spaces/m21hvnsjmt8a/assets/2dNx7g19AokMyKq4e46guk');
      ...
      it('model "asset2"', function () {
        let type, id;

        type = store.modelFor('asset');
        id = '2dNx7g19AokMyKq4e46guk';
        //adapter.findRecord(store, type, id, null);
        //return expect(adapter.ajax).to.eventually.ok;

        //return assert.isFulfilled(promise, "optional message");

        var promise = new Promise(function (resolve) {
          setTimeout(resolve, 200, { foo: "bar" } );
        });

        var i = { foo: "bar" }
        expect(i).to.eventually.equal(4);
